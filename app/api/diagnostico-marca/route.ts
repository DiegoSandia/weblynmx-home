...
function buildHtml(d: Record<string, string>) {
  const row = (label: string, val: string) => `
    <tr>
      <td style="padding:10px 16px;font-size:12px;color:#888;white-space:nowrap;width:180px;vertical-align:top;border-bottom:1px solid rgba(255,255,255,0.04);">${label}</td>
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Diagnóstico de Marca</title></head>
  ...
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    <div style="text-align:center;margin-bottom:28px;">
      ...
      <h1 style="font-size:22px;font-weight:700;color:#F5F5F5;margin:0 0 4px;">Nuevo Diagnóstico de Marca</h1>
    ...
    ${section('Identidad de marca',
      row('¿Cómo describirías tu marca?', d.descripcionMarca) +
      row('¿Qué percepción quieres transmitir?', d.percepcion) +
      row('¿Qué marcas te inspiran?', d.marcasInspiran) +
      row('Problema con la imagen actual', d.problemaImagen)
    )}
    ${section('Tu negocio y cliente',
      row('¿Qué hace diferente tu negocio?', d.diferenciador) +
      row('¿Quién es tu cliente ideal?', d.clienteIdeal) +
      row('¿Qué dudas tiene antes de comprarte?', d.dudasCliente)
    )}
...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      ...
      subject: `Diagnóstico de Marca · ${data.nombreNegocio || 'Nuevo lead'}`,
      html: buildHtml(data),
    });

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error('[api/diagnostico-marca]', err);
